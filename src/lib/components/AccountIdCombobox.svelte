	...
	function getFilteredOptions() {
		...
		return entries
			.filter((entry) => {
				...
				const roster = entry[1];
				return (
					accountId.toString().includes(searchTerm) ||
					roster.player.name.toLowerCase().includes(searchTerm)
				);
			})
			.slice(0, 10);
	}
...
<div class="relative">
	...
	{#if showDropdown && getFilteredOptions().length > 0}
		...
		>
			{#each getFilteredOptions() as entry}
				{@const accountId = entry[0]}
				{@const roster = entry[1]}
				<button
					type="button"
				...
				>
					<div class="flex justify-between">
						...
						<span class="text-slate-400">{roster.player.name}</span>
					</div>
				</button>
			{/each}
		</div>
