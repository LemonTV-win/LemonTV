...
<script lang="ts">
	...
	function removeVideo(index: number) {
		newEvent.videos = newEvent.videos.filter((_, i) => i !== index);
	}

	function updateVideo(index: number, field: 'type' | 'platform' | 'url' | 'title', value: string) {
		newEvent.videos = newEvent.videos.map((v, i) => (i === index ? { ...v, [field]: value } : v));
	}
	...
	>
		...
		<div class="flex flex-col gap-4 p-4">
			...
			<div class="flex flex-col gap-4">
				{#each newEvent.videos as video, i}
					<div class="flex flex-col gap-2 rounded-lg border border-slate-700 bg-slate-800 p-4">
						...
						<div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
							<div class="flex flex-col gap-2">
								<label for={`video-type-${i}`} class="text-sm font-medium text-slate-300"
									>Type</label
								>
								<select
									id={`video-type-${i}`}
