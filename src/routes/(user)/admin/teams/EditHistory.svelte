<script lang="ts">
	...
	async function loadHistory() {
		...
		error = null;
		try {
			const response = await fetch(`/api/teams/${team.id}/history`);
			if (!response.ok) {
...
<div class="space-y-6 pr-2">
	...
	{:else}
		...
		<div class="rounded-lg border border-slate-800 bg-slate-900/95 p-4 shadow-lg">
			<div class="flex items-center gap-2">
				<h2 class="text-xl font-medium text-slate-200">{team.name}</h2>
				...
				<!-- {#if team.players && team.players.length > 0}
					<span class="text-slate-400">
						({team.players.length}
						{team.players.length === 1 ? m.player() : m.players()} : {team.players
